...
export class LevelsService {
  ...
  async create(data: CreateLevelDto) {
    return this.prisma.loyaltyLevel.create({ data: data as any });
  }
  ...
  async update(id: string, data: UpdateLevelDto) {
    const level = await this.prisma.loyaltyLevel.findUnique({ where: { id } });
    if (!level) throw new NotFoundException('Level not found');
    return this.prisma.loyaltyLevel.update({ where: { id }, data: data as any });
  }
  ...
  async getMembershipLevel(membershipId: string) {
  ...
  async enableLevelsForMerchant(merchantId: string) {
  ...
  async disableLevelsForMerchant(merchantId: string) {
    return this.prisma.merchant.update({
      where: { id: merchantId },
  ...
  async getMerchantLevelsStatus(merchantId: string) {
  ...
  async evaluateAndUpgrade(membershipId: string) {
    const membership = await this.prisma.membership.findUnique({
      where: { id: membershipId },
    ...
    if (!membership.merchant.levelsEnabled) return null;
    ...
    if (bestLevel && bestLevel.id !== membership.levelId) {
      ...
      // Incrementar skinVersion en wallet passes activos
